...
FILE_PATH = "/Users/demetresasbestas/Desktop/ΔΙΠΛΩΜΑΤΙΚΗ/living_door/LivingRoom_Contact_Exported_202501301236.csv"
OUTPUT_NAME = "clean_living_door_data.csv"
...
df_door = df_door.rename(columns={'trntime': 'ds', 'value': 'living_door_status'})
...
df_door = df_door.set_index('ds').resample('1h')['living_door_status'].mean().to_frame()
...
df_door['living_door_status'] = df_door['living_door_status'].ffill().bfill()
#Clipping values under 0 or over 1
df_door['living_door_status'] = df_door['living_door_status'].clip(lower=0)
df_door['living_door_status'] = df_door['living_door_status'].clip(upper=1)
...
plt.plot(df_door['ds'], df_door['living_door_status'], color='green', drawstyle='steps-post')
plt.title("Living Room Door Status Over Time (1=Open, 0=Closed)")
...
plt.fill_between(subset['ds'], subset['living_door_status'], color='lightgreen', alpha=0.5)
plt.plot(subset['ds'], subset['living_door_status'], color='darkgreen', marker='o', markersize=3)
plt.title("Detailed View: Living Room Door Activity (First 100 Hours)")
plt.xlabel("Date/Time")
plt.ylabel("Status")
